...
現場実務として約1年半、TypseScript言語での比較的小規模なWebアプリ/システムの開発業務に従事しました。**フロントエンドFE, バックエンドBE, インフラ層などの実装に必要な範囲全てを、基本的に単独で担当**してきました（開発職の先輩が在籍中は適宜相談してご協力いただきました）。具体的な主業務は社内ディレクターからの**要求整理、技術選択、各種設計、実装、公開、継続保守、ドキュメンテーションなど**です。加えてその他業務として、全社Web制作案件ごとにPaaSインフラ立ち上げとDNS管理、API仕様変更や脆弱性リリースへの対応、既存案件の引き継ぎと保守、社内ブログの執筆、エンジニア採用面接なども並行して担当していました。
...
`Next.js AppRouter`でのWebアプリ開発ができそうな会社のみに絞った就活を経て、エンジニアとしてキャリアを始めた最初の会社です。TypseScript言語での比較的小規模なWebアプリ/システムの開発業務に従事しました。社内の慢性的な開発リソース不足もあり、FE, BE, インフラ層などの実装に必要な範囲全てを、基本的に単独で担当していました（開発職の先輩が在籍中は適宜相談してご協力いただきました）。最も印象深かったプロジェクトは入社直後のWebアプリ受託案件です。
...
ご依頼主の建築資材商社様の取引先ネットワークを活かした**業者および案件仲介Webアプリ**。発注業社（元請）と受注業者（下請）を、案件または会社単位でマッチングさせる2つの仲介機能を主に提供するものです。ユーザー画面側では会員登録、案件登録、案件検索・エントリー、ユーザー検索・マッチ申請などが可能。管理画面に反映させたそれら情報を元に、先方商社の営業担当が発注・受注双方に本格的に営業活動して仲介をクロージングする仕様のため、**実態はCRMアプリ** と言えそうです。
...
- 全社的にWeb制作案件が多いため、アプリ開発における社員のスキルや経験値、人数、工期など根本的なリソースは慢性的に不足。細部まで仕様が検討・ドキュメント化されるのは稀なので、機能開発はディレクターとその都度相談・自ら提案を重ねてタスクを進めていく必要がありました。
- テストコードおよびコードレビューなし。ローカルで確認後、各自セルフマージ（当時Git導入後まだ1年未満）
- TS/React + バックエンドも自社で開発するのは会社として初めて
- 同時期に別の中規模開発案件が開始したため、筆者は入社間もなく専任の開発者として本案件に参加
