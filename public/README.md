...
`Next.js AppRouter`でのWebアプリ開発ができそうな会社のみに絞った就活を経て、エンジニアとしてキャリアを始めた最初の会社です。TypseScript言語での比較的小規模なWebアプリ/システムの開発業務に従事しました。社内の慢性的な開発リソース不足もあり、FE, BE, インフラなどその開発に必要な範囲全てを、基本的に単独で主担当していました（開発職の先輩が在籍中は適宜相談して助けていただきました）。
...
前提: アプリ開発の実績・人員・スキル全てが乏しい弊社にとって、バックエンド開発も自社で行うほぼ初めての受託案件のようでした。しかし担当予定者の退職や別の中規模開発案件の同時開始もあり、急遽フルスタックWeb開発ポジションが募集され筆者が入社後即参加しました。

|役割|人数|担当業務|備考|
|---|---|---|---|
|ディレクター|1名|依頼企業様との打合せ、大まかな機能やページ構造などの検討・すり合わせ、進捗管理など|担当者の本業は動画制作とマークアップのため**アプリ開発およびディレクションは未経験**での挑戦。他の中規模開発案件のディレクションやWebサイトマークアップ業務とも並行して担当。本プロジェクト中盤で途中退職後、社長が引継ぎ|
|デザイナー|2名|URLパスの策定、ワイヤーフレーム作成、デザインの作成など|ページごとに分担|
|開発エンジニア（筆者）|1名|下記セクションに詳述|**専任**で開発を主担当。|
|マークアップエンジニア|2名|ユーザー画面側の静的ページのマークアップ、CMS経由のブログ記事ページの実装、サイトマップやメタタグ等SEO関連実装、筆者が担当した機能系ページのHTMLセマンティクスやCSSの改善など|他プロジェクトとも並行|
|マーケターその他|2名|ページ内やメール送信時文言などの検討、マニュアル資料の作成など|他プロジェクトとも並行|
